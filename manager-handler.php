...
if ($action === 'add_product') {
    ...
    // Validation
    if (empty($title) || empty($description) || empty($price) || empty($image_url) || $stock === '') {
...
if ($action === 'edit_product') {
    ...
    try {
        $stmt = $pdo->prepare(
            'UPDATE products SET title = ?, description = ?, price = ?, image_url = ?, stock = ? WHERE id = ?'
        );
        $stmt->execute([$title, $description, $price, $image_url, $stock, $product_id]);
